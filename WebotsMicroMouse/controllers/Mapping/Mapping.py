...
class location():
    ...
    def rotate(self,radians):                                                 ########
        ...
        #print("Starting direction is {} and the end direciton is {} and the difference of change is {}".format(start_direction, end_direction,radians))
        leftMotor.setVelocity(0)
        rightMotor.setVelocity(0)
...
while robot.step(timestep) != -1:
    ...
    if(leftMotor.getVelocity() == rightMotor.getVelocity() and Backtrack_Option == False):
        ...
        print("Before: The current is {}. The previous is {} for one wheel".format(current[0],previous[0]))
        if difference[0] >= 8.83:                                   #this if statment determines wehter or not the robot move cells
            ...
            if( left_dist > .18):                                 #Create Additional conditions
                ...
                print("this turns left")
            elif(right_dist>.18):
                 Robot_Pose.optionalturn("right")
                 print("this turns right") 
            elif(front_dist<.18 and left_dist<.18 and right_dist<.18):
                print("the robot is headin in this direction {}".format(round(imu.getRollPitchYaw()[2],2)))      
                Robot_Pose.rotate(3.14)
                print("There was a point when the object reached a dead end") 
                temp = stack.pop()
                tmep = stack.pop()
            ...
            print("After: The current is {}. The previous is {} for one wheel".format(current[0],previous[0]))
        ...
        if difference[0] >= 8.83:  
            ...
            temp = stack.pop()
            print("Inside the CURRENT the row is {} and the columns is {}".format(Robot_Pose.location_row,Robot_Pose.location_column))
           # print("Inside the STACK the top row is {} and the top column is {}".format(temp.Row, temp.Column))
            if(maze.availability() == True):
                print("The next coordinates are row: {} column: {}".format(Robot_Pose.next_cell.Row, Robot_Pose.next_cell.Column))
                Backtrack_Option = False
                leftMotor.setVelocity(0)                    #stops the robot so it can continue other stuff
                rightMotor.setVelocity(0)
            else:
...
while(count_row < 16):
    while(count_column < 16):
        print("row is {}. Column is {}".format(count_row, count_column))
...
print("the code has discovered {} cells".format(new_count))
